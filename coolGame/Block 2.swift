...
class Block: Entity {
    ...
    func initColor() {
        ...
        if(type == 0 || type == 1 || (type == 3 && colorIndex == -1) || (type == 4 && colorIndex == -1) || (type == 7 && colorIndex == -1)) {
            let blockVariation = Int(rand()*Board.colorVariation)
            
            if(type == 0 || ((type == 3 || type == 4 || type == 7) && colorIndex == -1)) {
    ...
    func getInverseColor(index: Int) -> UIColor {
        if(type == 0 || type == 1 || (type == 3 && index == -1) || (type == 4 && index == -1) || (type == 7 && index == -1)) {
            let blockVariation = Int(rand()*Board.colorVariation)
            
            if(type == 0 || ((type == 3 || type == 4 || type == 7) && index == -1)) {
                if(!GameState.inverted) {
            ...
            let n = 0.75
            let blockVariation = Int((Board.colorVariation*n) - (rand()*Board.colorVariation*2*n))
            var colorArray = ColorTheme.colors[Board.colorTheme][index]
    ...
    override func updateSprite() {
        if(GameState.state != "inverting") {
            if(type == 4) {
                ...
                let otherColor = UIColor.init(red: CGFloat(r), green: CGFloat(g), blue: CGFloat(b), alpha: 1.0)
            ...
            if(GameState.getInversionLinePosition() <= x) {
                ...
                if(GameState.getPrevInversionLinePosition() < x+1) {
                    ...
                    s.fillColor = getInverseColor(index: colorIndex)
    ...
    func beginInversion() {
        sprite[0].addChild(SKShapeNode.init())
    }
}
